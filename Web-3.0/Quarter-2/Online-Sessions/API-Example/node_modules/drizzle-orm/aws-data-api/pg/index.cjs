...
var _a$1, _b, _c;
class AwsDataApiPreparedQuery extends alias.PreparedQuery {
...
class AwsDataApiSession extends alias.PgSession {
    constructor(
    /** @internal */
...
class AwsDataApiTransaction extends alias.PgTransaction {
    transaction(transaction) {
        const savepointName = `sp${this.nestedIndex + 1}`;
        const tx = new AwsDataApiTransaction(this.dialect, this.session, this.schema, this.nestedIndex + 1);
        this.session.execute(alias.sql `savepoint ${savepointName}`);
        try {
            const result = transaction(tx);
            this.session.execute(alias.sql `release savepoint ${savepointName}`);
            return result;
        }
        catch (e) {
            this.session.execute(alias.sql `rollback to savepoint ${savepointName}`);
            throw e;
        }
    }
}
...
var _a;
class AwsPgDialect extends alias.PgDialect {
    escapeParam(num) {
        return `:${num + 1}`;
    }
}
