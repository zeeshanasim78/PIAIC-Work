...
var session = require('../session-805d2876.cjs');
                ...
                if (row === undefined || (Array.isArray(row) && row.length === 0)) {
                    ...
                    if (hasRowsMapper) {
                        if (customResultMapper) {
                            ...
                            yield alias.mapResultRow(fields, row, joinsNotNullableMap);
                        }
                    }
                    else {
                        yield row;
...
class MySql2Session extends session.MySqlSession {
    ...
    async transaction(transaction, config) {
        ...
        if (config) {
            ...
            await (startTransactionSql ? tx.execute(startTransactionSql) : tx.execute(alias.sql `begin`));
        }
        else {
            await tx.execute(alias.sql `begin`);
        }
        try {
...
function drizzle(client, config = {}) {
    ...
    if (config.schema) {
        if (config.mode === undefined) {
            throw new alias.DrizzleError('You need to specify "mode": "planetscale" or "default" when providing a schema. Read more: https://orm.drizzle.team/docs/rqb#modes');
        }
    ...
    const session$1 = driver.createSession(schema, mode);
    return new session.MySqlDatabase(dialect, session$1, schema, mode);
}
