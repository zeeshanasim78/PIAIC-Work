...
var session = require('../session-e9306785.cjs');
...
class PreparedQuery extends session.PreparedQuery {
    ...
    async all(placeholderValues) {
        ...
        return rows.map((row) => {
            return alias.mapResultRow(fields, Array.prototype.slice.call(row).map((v) => normalizeFieldValue(v)), joinsNotNullableMap);
        });
    }
    async get(placeholderValues) {
        ...
        return alias.mapResultRow(fields, Array.prototype.slice.call(rows[0]).map((v) => normalizeFieldValue(v)), joinsNotNullableMap);
    }
