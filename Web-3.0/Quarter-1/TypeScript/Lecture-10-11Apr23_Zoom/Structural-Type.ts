...
interface Sphere {
  ...
  // We can also use type insted of interface , both are used to create type and
  // there is a minor difference between then which we will not discuss at this time
/* For Example :
 interface Ball {
    diameter: number; }
*/ 

 // We can also define Inline Types as 
 // let ball: {diameter:number} = { diameter: 10 };
 // let sphere: {diameter:number} = { diameter: 20 };
